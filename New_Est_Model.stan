data {
	...
	int<lower=0> T;  // number of time points
  	...
  	matrix [p,p] R_s[S];     // array with matrices R_s for all subjects
	matrix [q,p] E_s[S];     // array with matrices E_s for all subjects
	matrix [q,q] Q_s_inv[S]; // array with matrices Q_s_inv for all subjects
...
model {
	...
	target += S*( lmgamma(p,0.5*(T+nu)) - lmgamma(p,0.5*nu) ) + 0.5*S*nu*log_determinant(nu*Sigma) - 0.5*(nu+T)*Sum_logdet; // add log-likelihood term to the posterior
}
